...
**Slow Animations** (Debug-меню симулятора) - запасной вариант, если нужно разглядеть детали моушена. Пункт меню кликом не берётся (computer-use считает его чужим приложением) - открывай меню кликом по «Debug», потом `Down` + `Return`, и **проверяй галочку зумом**: с первого раза может не переключиться. Не забудь выключить обратно.
